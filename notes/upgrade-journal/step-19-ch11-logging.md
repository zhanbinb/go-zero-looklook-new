...
filebeat 抓 go-zero 业务日志 (host 上 tmp/logs/*.log)
kafka 做缓冲 (topic: looklook-log)
go-stash 从 kafka 消费, 过滤, 写 ES (索引: looklook-YYYY-MM-DD)
Kibana 可视化 (data view: looklook-*)
承接 step-17 (ch 13 监控) + step-18 (ch 12 追踪), 是 ch 11/12/13 三件套的第三章.
§1 起点 - 项目现状
...
根因: filebeat 原本只配 paths /var/lib/docker/containers/*/*-json.log (docker 容器), 但我们的业务服务跑在 host 上, filebeat 在容器内看不到 host 的 tmp/logs/*.log.
...
docker restart 不行! 端口映射/挂载是创建容器时定的, 重建才行.
...
左侧 ☰ → Stack Management → Data Views
点 Create data view (右上角)
填写:Index pattern: looklook-* (通配符匹配所有日期索引)
Timestamp field: @timestamp (filebeat 自带, ISO 8601 格式)

点 Create data view
左侧 → Discover → 顶部选刚建的 looklook-*
看到所有日志 (按时间倒序)
...
7.5 常见 3 步排查流程
筛选时间范围: 选 1h / 15min / 自定义
KQL 过滤: 比如 level: "error" AND log_type: "looklook_business"
展开看 tags: 点击日志行左侧箭头, 看完整字段
§8 总结
8.1 5 步走成果
步骤	内容	状态
...
原始教程 [ch 11-日志收集](doc/chinese/11-日志收集.md) (filebeat 7.x 旧版, 我们升到 8.12 适配)
创建于 2026-08-11, ch 11 日志收集完整跑通 (filebeat 抓 host 业务日志 + Kibana data view looklook- 看所有日志), v3.37 commit 已 ship*
EOF
echo "===== 文档结束 ====="
echo "长度: ~510 行"